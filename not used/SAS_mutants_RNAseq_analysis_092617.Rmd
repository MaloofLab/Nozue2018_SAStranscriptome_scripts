...
save(PLoSGenetics2015mutants.errrDE,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","PLoSGenetics2015mutants.errrDE.Rdata")) # this has to be used below. Replace "samples.nolow5" into "PLoSGenetics2015mutants.errrDE" 
...
Genotype<-levels(as.factor(sub("(C|D|E)([[:digit:]]+)(L|H)(1|4|25|49)(A.merged.bam)","\\2",names(SAS.expression.vst)))) # 13 genotypes inclucing Col. better to use samples.nolow5.Rdata info (confusing)
#
for(i in Genotype) { # why I eliminate genotype 1 and 10???
  ...
  save(mds.dge,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h",paste("MDS_with_genotype_wo_outlier_wo16h",i,".RData",sep="")))
...
```
# MDS plot by ggplot2 (100417)
```{r}
mds.files<-list.files(pattern="^MDS_with_genotype_wo_outlier_wo16h",path=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h"))
for(i in mds.files) {
...
ggsave(p.distance_matrix.mds.dge, filename =file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h",paste("p.distance_matrix.mds.dge.",vlookup(gsub("(MDS_with_genotype_wo_outlier_wo16h)([[:digit:]]+)","\\2",i),conversion.table,2),".png",sep="")), height = 11, width = 25) 
}
...
dim(DGEsummary.edgeR.response) #755   8 for final sequence data with normalization (081514). This is whitney. but my computer is 505 8.
...
length(DGEgenelist.edgeRglm) # 418 > 363 (092817)
# 
...
save(DGE.summary.vst.response.kazu.largeCV,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","DGE.summary.vst.response.kazu.largeCV.Rdata")) # updated 052916, 092517
...
```
# write summary of GOseq analsyis of SOM clusters (022215; 100617)
```{r eval=FALSE}
#rm(list=ls())
...
write.csv(SOM.Col.cluster.GOseq.summary,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","TableS4_GO_enrichment_analysis_of_co-expressed_SOM_clusters_of_shade-responsive_genes092617.csv")) # 091016,092617, 100617
```
...
save(overlap.GOseq.melt,file=file.path(homedir,"..","..","Nozue2016_SAStranscriptome_output","output",paste("overlap.GOseq.melt.SOM.mut.allgene.",i,".Rdata",sep="")))
for(i in 3:(dim(test)[2]-1)) { # the last column is "ssom3.4.unit.classif"
overlap.GOseq<-GOseq.overlapTable(test[,"ssom3.4.unit.classif"],test[,i],genes=test$AGI)
overlap.GOseq.melt<-melt(overlap.GOseq$pTable,id=1) #
overlap.GOseq.melt$count<-melt(overlap.GOseq$countTable,id=1)$value
overlap.GOseq.melt<-overlap.GOseq.melt[!overlap.GOseq.melt$Var2=="0",]
#overlap.GOseq.melt.summary<-rbind(overlap.GOseq.melt.summary,overlap.GOseq.melt)
save(overlap.GOseq.melt,file=file.path(homedir,"..","..","Nozue2016_SAStranscriptome_output","output",paste("overlap.GOseq.melt.SOM.mut.all.gene",i,".Rdata",sep="")))
}
```
...
overlap.GOseq.melt.SOM.mut.files<-list.files(path=file.path("..","..","Nozue2016_SAStranscriptome_output","output"),pattern="overlap.GOseq.melt.SOM.mut.all.gene") # for all gene as background (92717)
...
```
# drawing overlapTable heatmap (SOM vs mutant affected genes) (092717) 
```{r}
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","overlap.GOseq.melt.summary.SOM.DEgenes.092817.Rdata"))
...
ggsave(q4b,file="Fig4_Shade_avoidance_mutants_misexpress_specific_shade-response_SOM_cluster_092817.png",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") 
ggsave(q4b,file="Fig4_Shade_avoidance_mutants_misexpress_specific_shade-response_SOM_cluster_092817.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") 
q4b 
```
# Overlap analysis with custome categories (SOM)
## making summary table/dataset 
```{r eval=FALSE}
...
# SOM clusters
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","newdata.vst.Rdata")) # 092617
...
write.csv(hormone.responsiveness.SOMclusters,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","TableS4_hormone.responsiveness.SOMclusters092717.csv"))
...
write.csv(hormone.responsiveness.SOMclusters.description,file="../../Nozue2016_SAStranscriptome_output/figs_tables//Dataset2_Custom_enrichment_analysis_of_co-expressed_SOM_clusters_of_shade-responsive_genes092717.csv") 
```
## overlapTable (SOM vs custom category)
```{r}
test<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Dataset2_Custom_enrichment_analysis_of_co-expressed_SOM_clusters_of_shade-responsive_genes092717.csv"),row.names=1) # 
#test<-test[,-1]
# overlap table 
i<-2 
overlap.GOseq<-GOseq.overlapTable(test[,"ssom3.4.unit.classif"],test[,i],genes=test$AGI)
...
overlap.GOseq.melt<-overlap.GOseq.melt[!overlap.GOseq.melt$Var2=="0",]
save(overlap.GOseq.melt,file=file.path(homedir,"..","..","Nozue2016_SAStranscriptome_output","output",paste("overlap.GOseq.melt.SOM.customcat.",i,".Rdata",sep="")))
...
```{r}
overlap.GOseq.melt.SOM.customcat.files<-list.files(path=file.path("..","..","Nozue2016_SAStranscriptome_output","output"),pattern="overlap.GOseq.melt.SOM.customcat")
...
overlap.GOseq.melt.summary[overlap.GOseq.melt.summary$value>10,"value"]<-10
overlap.GOseq.melt.summary[overlap.GOseq.melt.summary$count==0,"count"]<-""
save(overlap.GOseq.melt.summary, file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","overlap.GOseq.melt.summary.SOM.customcat.092817.Rdata") )
```
...
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.092817.pdf",width=13,height=6,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"))
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.092817.png",width=13,height=6,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables")) # 
```
...
# overlap between WGCNA Col modules and mutation affected genes (needs to run)
```{r,eval=FALSE}
#setwd(homedir)
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","DE.genotype.batch.genes.unique.DF.Rdata")) #[1] 3350   25
#setwd(homedir)
...
#load(file.path("..","Nozue2016_SAStranscriptome_output","output","SAS.expression.vst.s.kazu.largeCV.modules121316.Rdata"))
...
write.csv(DE.genotype.batch.genes.WGCNAmodules.description,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Dataset7_Complete_gene_list_of_genes_misexpressed_in_mutant_and_found_in_WGCNA_modules_visualized_in_Fig7_092817.csv")) #091616. Column names were screwed up (111616). Orange > "salmon", "salmon" > "orange4" (121316), 092817
...
test<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Dataset7_Complete_gene_list_of_genes_misexpressed_in_mutant_and_found_in_WGCNA_modules_visualized_in_Fig7_092817.csv"),row.names=1) # does  work (092717)

test[ gsub("([[:digit:]]+)(\\.)([[:digit:]]+)","\\1",test$AGI) %in% c("AT3G22231","AT4G14365","AT1G33960","AT4G14400"),] # OK

#test<-test[,-1]
dim(test) # 17985   28 > 8156   28
...
#overlap.melt.summary<-overlap.melt[!overlap.melt$Var2=="0",]
overlap.GOseq.melt<-overlap.GOseq.melt[!overlap.GOseq.melt$Var2=="0",]
save(overlap.GOseq.melt,file=file.path(homedir,"..","..","Nozue2016_SAStranscriptome_output","output",paste("overlap.GOseq.melt.WGCNA.mut.",i,".Rdata",sep="")))
##### under construction #### (092817)
for(i in 3:25) { #
  overlap.GOseq<-GOseq.overlapTable(test[,"WGCNAmodules"],test[,i],genes=test$AGI)
  overlap.GOseq.melt<-melt(overlap.GOseq$pTable,id=1) #
  overlap.GOseq.melt$count<-melt(overlap.GOseq$countTable,id=1)$value
...
# combine 
```{r}
overlap.GOseq.melt.WGCNA.mut.files<-list.files(path=file.path("..","..","Nozue2016_SAStranscriptome_output","output"),pattern="overlap.GOseq.melt.WGCNA.mut")
...
# reading and cleaning overlap table and graph (WGCNA vs mut)
```{r}
...
```
## write summary of WGCNA.Col.module.GOseq (modified 092717)
```{r eval=FALSE}
...
write.csv(WGCNA.Col.module.GOseq.summary,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Dataset5_GO_enrichment_analysis_of_WGCNA_modules092817.csv")) # Dataset 5. GO enrichment analysis of WGCNA gene co-expression modules (091716, salmon4<>orange 121316, rerun 091817; 092817)
```
...
write.csv(hormone.responsiveness.WGCNAmodules,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Supplemental_Dataset_6_Custom_category_enrichment_analysis_of_WGCNA_gene_co-expression_modules092817.csv"))
...
hormone.responsiveness.WGCNAmodules[hormone.responsiveness.WGCNAmodules$IAAup %in% "IAAup" & hormone.responsiveness.WGCNAmodules$WGCNAmodules=="green", c("AGI","IAAup","IAAdown","SARminus","SARplus","NPR1down","WRKY33down","WRKY33up","SAup_24h","SAdown_24h","NPR1up_uponSA","NPR1down_uponSA","WGCNAmodules")]

```
# overlapTable for WGCNA module vs custome categories (100417)
```{r}
hormone.responsiveness.WGCNAmodules<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Supplemental_Dataset_6_Custom_category_enrichment_analysis_of_WGCNA_gene_co-expression_modules092817.csv"),row.names=1)
hormone.responsiveness.WGCNAmodules[hormone.responsiveness.WGCNAmodules$WGCNAmodules %in% "salmon" & hormone.responsiveness.WGCNAmodules$AGI %in% c("AT3G22231","AT4G14365","AT1G33960","AT4G14400"), c("AGI","IAAup","IAAdown","SARminus","SARplus","NPR1down","WRKY33down","WRKY33up","SAup_24h","SAdown_24h","NPR1up_uponSA","NPR1down_uponSA","WGCNAmodules")]
# OK
...
#overlap.melt.summary<-overlap.melt[!overlap.melt$Var2=="0",]
overlap.GOseq.melt<-overlap.GOseq.melt[!overlap.GOseq.melt$Var2=="0",]
save(overlap.GOseq.melt,file=file.path(homedir,"..","..","Nozue2016_SAStranscriptome_output","output",paste("overlap.GOseq.melt.WGCNA.customcat.",i,".Rdata",sep="")))
#### under construction ####
for(i in 3:32) {
  overlap.GOseq<-GOseq.overlapTable(test[,"WGCNAmodules"],test[,i],genes=test$AGI)
  overlap.GOseq.melt<-melt(overlap.GOseq$pTable,id=1) #
...
```
# combine files (WGCMA vs customcat)
```{r}
# read files
...
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","summary.vst.response.Rdata")) # response. needs to fix colnames (060515), (092917)
#summary.vst.response.kazu<-summary.vst.response[,gsub("([[:digit:]]+)(_)(1|4|16|25|49)(hrA)","\\1",colnames(summary.vst.response)) %in% 1:19]
...
my.category.heatmap.graph[[i]]<-my.category.heatmap5(summary3.response=summary.vst.response.kazu,gt.num=i,gt2=conversion.table[conversion.table$num==i,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.092917v1.png",w=5,h=5*4/3, path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"),save.plot=F,legend.fill=T) # error
}
...
my.category.diff.data[[i]]<-my.category.diff(summary3.response=summary.vst.response.kazu,gt.num=i,gt2=conversion.table[conversion.table$num==i,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.092917muts.diff.v1.png",w=5,h=5*4/3, path="../../Nozue2016_SAStranscriptome_output/figs_tables/",save.plot=F,legend.fill=T) # make specialized for difference heatmap function
}
...
response.melt <- melt(summary.vst.response.kazu,id.vars= "AGI",variable.name = "sample", value.name = "log2.dif")
head(response.melt)
...
```{r}
response.p.values<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Fig5B_response_p_values.v3.csv"),row.names=1) # from modified Julin's script (copied from Kazu_test_v2.R)
...
save_plot(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Fig5_all.my.category.diff.expression.pattern.v7.100417.FDR0.01.png"), my.category.heatmap.all,
...
#
#setwd("../../Nozue2016_SAStranscriptome_output/figs_tables/")
save_plot(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Fig_S2_all.my.category.expression.pattern.v12.100417.png"), my.category.heatmap.all,
          ncol = 3, # we're saving a grid plot of 2 columns
          nrow = 5, # and 2 rows
          base_aspect_ratio = 2.5,
          base_height=6,
...
test<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","TableS4_hormone.responsiveness.SOMclusters092717.csv") ,row.names=1) # needs to use updated one (092917)
#test<-test[,-1]
...
write.csv(test.wozero.melt.hormone.significant.ss,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","TableS7_test.wozero.melt.hormone.significant.ss.092917.csv")) # 
...
test.wozero.melt.hormone.significant.ss.summary
#setwd("../../Nozue2016_SAStranscriptome_output/output/")
...
write.csv(test.wozero.melt.hormone.significant.sss,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS7_test.wozero.melt.hormone.significant.sss.092917.csv") # 
