...
set.seed(2) # Set a random seed so that SOM results are reproducible
  for(i in 1:10) {
    ssom1.5 <- som(as.matrix(data5), somgrid(1,5,"hexagonal"))
    ...
    ssom2.3 <- som(as.matrix(data5), somgrid(2,3,"hexagonal")) 
    plot(ssom2.3, type = "codes")
    ssom1.7 <- som(as.matrix(data5), somgrid(1,7,"hexagonal")) 
    plot(ssom1.7, type = "codes")
    ssom2.4 <- som(as.matrix(data5), somgrid(2,4,"hexagonal")) 
    plot(ssom2.4, type = "codes")
    ssom3.3 <- som(as.matrix(data5), somgrid(3,3,"hexagonal")) 
    plot(ssom3.3,type="codes")
    ssom2.5 <- som(as.matrix(data5), somgrid(2,5,"hexagonal")) 
    plot(ssom2.5, type = "codes") #
    ssom3.4 <- som(as.matrix(data5), somgrid(3,4,"hexagonal")) 
    plot(ssom3.4) 
    ssom4.4 <- som(as.matrix(data5), somgrid(4,4,"hexagonal")) 
...
# draw heatmap
```{r}
  overlap.melt.SOM.customcat.PLoSGenetics2015mutants.errrDE.summary.files<-list.files(file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap"),pattern="overlap.melt.SOM.customcat.PLoSGenetics2015mutants.errrDE.summary")

for(y in overlap.melt.SOM.customcat.PLoSGenetics2015mutants.errrDE.summary.files) 
  {
  load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap",y))
# graph
...
ggsave(q4b,file=paste("Fig3_Col.SOM.clusters.vs.my.category.table.s.2.set.",title,".pdf",sep=""),width=13,height=6,path=file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap"))
}
```
...
```{r}
# set1
GOseq.SOM(som="1.5",set="1")
GOseq.SOM(som="2.3",set="1")
GOseq.SOM(som="1.7",set="1")
GOseq.SOM(som="2.4",set="1")
GOseq.SOM(som="3.3",set="1")
GOseq.SOM(som="2.5",set="1")
GOseq.SOM(som="3.4",set="1")
GOseq.SOM(som="4.4",set="1")


```


# GOseq anaysis of SOM clusters (som1.5.set1 sepecific)
```{r}
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap","ssom.summary.set.files.DF.Rdata"))
ssom.summary.set.files.DF$gt<-gsub("(AT[[:alnum:]]+)(\\.)([[:digit:]]+)","\\3",ssom.summary.set.files.DF$AGI)
ssom.summary.set.files.DF.Col<-ssom.summary.set.files.DF[ssom.summary.set.files.DF$gt==1,]
...
clusters<-as.numeric(levels(as.factor(ssom.summary.set.files.DF.Col$"1.5.set.1")))
# modify GOseq.ORA if there is no enriched GO ("return" instead of "stop") (070716)
for(x in clusters) {  
  temp.GOseq<-GOseq.ORA(ssom.summary.set.files.DF.Col[ssom.summary.set.files.DF.Col$"1.5.set.1"==x,"AGI"])
  if(temp.GOseq=="no enriched GO") {next} else {
     temp.genes<-genes.in.enriched.category(enrich.result=temp.GOseq,gene.list=ssom.summary.set.files.DF.Col[ssom.summary.set.files.DF.Col$"1.5.set.1"==x,"AGI"],category.table=Atgo.list)
  save(temp.GOseq,temp.genes,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap",paste("1.5.set.1","SOM.Col.cluster.",x,".GOseq.PLoSGenetics2015mutants.errrDE.Rdata",sep="")))
  } 
}
```
# write summary of GOseq analsyis of SOM clusters 
```{r}
SOM.Col.cluster.GOseq.files<-list.files(pattern="SOM.Col.cluster.",path=file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap"))
SOM.Col.cluster.GOseq.files<-grep(".GOseq.PLoSGenetics2015mutants.errrDE.Rdata",SOM.Col.cluster.GOseq.files,value=TRUE)
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap",SOM.Col.cluster.GOseq.files[1]))
temp.GOseq$clusters<-gsub("(1.5.set.1SOM.Col.cluster.)([[:print:]]+)(.GOseq.PLoSGenetics2015mutants.errrDE.Rdata)","\\2",SOM.Col.cluster.GOseq.files[1])
temp<-temp.GOseq
for(i in SOM.Col.cluster.GOseq.files[-1]) {
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap",i))
temp.GOseq$clusters<-gsub("(1.5.set.1SOM.Col.cluster.)([[:print:]]+)(.GOseq.PLoSGenetics2015mutants.errrDE.Rdata)","\\2",i)
  temp<-rbind(temp,temp.GOseq)
}
SOM.Col.cluster.GOseq.summary<-temp
write.csv(SOM.Col.cluster.GOseq.summary,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","SOM_overlapTable_heatmap","som1.5.set.1.GO.csv")) 
```
