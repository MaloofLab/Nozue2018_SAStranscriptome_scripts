...
p <- p + coord_flip()
p 
ggsave("Fig2_Coexpression_clusters_of_shade-responsive_genes_090116.pdf",width=11,height=5,unit="in",path="../../Nozue2016_SAStranscriptome_output/figs_tables") # needs to change order of cluster (090116). File name changed (091016)
...
save(overlap.GOseq.melt.summary, file="../../Nozue2016_SAStranscriptome_output/output/overlap.GOseq.melt.summary.SOM.DEgenes.091016.Rdata") # 091516
load("../../Nozue2016_SAStranscriptome_output/output/overlap.GOseq.melt.summary.SOM.DEgenes.091016.Rdata")
# # graph
...
ggsave(q4b,file="Fig4_Shade_avoidance_mutants_misexpress_specific_shade-response_SOM_cluster_091616.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.091216.Rdata") # whitney. 
####### under construction #########
setwd(homedir)
setwd("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.091216.Rdata")
...
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.061816.png",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") # testing this with Whitney (091616)
...
for(y in moduleColors.files[-7]) {
  load(y)
  print(length(moduleColors)) # [1] 18379??? why?
...
setwd(homedir)
save(SAS.expression.vst.s.kazu.largeCV.modules,file="../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules.Rdata") #091616
```
...
setwd(homedir)
#rm(list=ls())
# under construction to eliminate "bsk5_1" (081816)

# # sun
# setwd("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/genotype_specific5")
# DE.genotype.batch.sun.files<-list.files(pattern="DE.genotype.batch.sun") # needs to select "sun" object (060615)
# #DE.genotype.batch.sun.files.kazu<-DE.genotype.batch.sun.files[gsub("([[:print:]]+)(wo16h)([[:digit:]]+)(.Rdata)","\\3",DE.genotype.batch.sun.files) %in% 1:19]
# DE.genotype.batch.sun.files.kazu<-DE.genotype.batch.sun.files[gsub("([[:print:]]+)(wo16h)([[:digit:]]+)(.Rdata)","\\3",DE.genotype.batch.sun.files) %in% Genotype]
# 
# load(DE.genotype.batch.sun.files.kazu[1])
# DE.genotype.batch.sun.genes<-rownames(DE.batch3.gt)
# for(xx in DE.genotype.batch.sun.files.kazu) {
...
# 
# #DE.genotype.batch.shade.files.kazu<-DE.genotype.batch.shade.files[gsub("([[:print:]]+)(wo16h)([[:digit:]]+)(.Rdata)","\\3",DE.genotype.batch.shade.files) %in% 1:19]
# DE.genotype.batch.shade.files.kazu<-DE.genotype.batch.shade.files[gsub("([[:print:]]+)(wo16h)([[:digit:]]+)(.Rdata)","\\3",DE.genotype.batch.shade.files) %in% Genotype]
# load(DE.genotype.batch.shade.files.kazu[1])
# DE.genotype.batch.shade.genes<-rownames(DE.batch3.gt)
# for(xx in DE.genotype.batch.shade.files.kazu) {
#   load(xx)
...
# ######
# DE.genotype.batch.genes.unique<-unique(c(DE.genotype.batch.sun.genes,DE.genotype.batch.shade.genes))
# length(DE.genotype.batch.genes.unique) #3356 > 1912 (only Genotype in new vst)
# # 
# DE.genotype.batch.genes.unique.DF<-data.frame(AGI=DE.genotype.batch.genes.unique)
# # sun
# for(x in DE.genotype.batch.sun.files.kazu) {
...
names(DE.genotype.batch.genes.WGCNAmodules)<-sub("X1","WGCNAmodules",names(DE.genotype.batch.genes.WGCNAmodules))
...
length(table(DE.genotype.batch.genes.WGCNAmodules$WGCNAmodules)) $ 59 modules
# adding gene description
# TAIR10_gene_descriptions<-read.csv("../../Nozue2016_SAStranscriptome_data/input/TAIR10_functional_descriptions.csv") 
# TAIR10_gene_descriptions$AGI<-gsub("([[:print:]]+)(.[[:digit:]]+)","\\1",TAIR10_gene_descriptions$"Model_name")
...
# graph
#overlap.melt.summary.s<-overlap.melt.summary[!overlap.melt.summary$Var2 %in% c("bsk5_1.sun","bsk5_1.shade","AT5G02540_1.sun","AT5G02540_1.shade"),]
save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.WGCNA.mut.GOseq.Rdata") # 091716 pending
...
load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules.Rdata") # is this current version? yes. (100716)
SAS.expression.vst.s.kazu.largeCV.modules$AGI<-gsub("([[:print:]]+)(.[[:digit:]]+)","\\1",SAS.expression.vst.s.kazu.largeCV.modules$AGI)
# merge with WGCNA modules
...
```
# appendix 1: differentially expressed genes used for ORA
```{r eval=FALSE}
