...
for(x in Genotype) {
  ...
  datExpr <-t(SAS.expression.vst.s.kazu.largeCV.x)
  load(paste("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.Rdata",x,".Rdata",sep=""))
...
merge.net3<-mergeCloseModules(
  ...
  consensusQuantile = 0,
  # Merging options
  cutHeight = 0.55, # smaller than 0.6
  iterate = TRUE,
  # Output options
  relabel = FALSE,
  colorSeq = NULL,
  ...
  verbose = 1, indent = 0)
# save parameters  
  moduleLabels = merge.net3$colors
  moduleColors = labels2colors(merge.net3$colors)
  MEs = merge.net2$MEs
  geneTree = merge.net3$dendrograms[[1]]
  save(MEs, moduleLabels, moduleColors, geneTree,file = paste("../../Nozue2016_SAStranscriptome_output/output/all",x,".largeCV.merge.net3.RData",sep="")) 
rm(net)
rm(merge.net3)
} # resultant ***
