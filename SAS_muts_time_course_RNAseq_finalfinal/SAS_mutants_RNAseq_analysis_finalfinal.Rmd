...
attributes(hormone.responsiveness5)
hormone.responsiveness5.DF<-data.frame(t(sapply(hormone.responsiveness5,c)))
names(hormone.responsiveness5.DF)<-attributes(hormone.responsiveness5)$category
## convert into hormone.responsiveness3.DF (omit "PCC1", "GRF1_3")
hormone.responsiveness5.DF<-hormone.responsiveness5.DF[,!names(hormone.responsiveness5.DF) %in% c("GRF1_3target","PCC1up","PCC1down")]
summary(hormone.responsiveness5.DF)
hormone.responsiveness5.DF$AGI<-rownames(hormone.responsiveness5.DF)
# SOM clusters
load("../../Nozue2016_SAStranscriptome_output/output/newdata.vst.Rdata") # 022215

newdata.vst.Col<-newdata.vst[newdata.vst$gt==1,] 
# merge with SOM clusters
test<-merge(hormone.responsiveness5.DF,newdata.vst.Col[,c("AGI","ssom3.4.unit.classif")],by="AGI",all=TRUE)
...
# ## addition for 27:30 (111916)
# i<-27
# overlap<-GOseq.overlapTable(test[,"ssom3.4.unit.classif"],test[,i],genes=test$AGI)
# overlap.melt<-melt(overlap$pTable,id=1) #
# overlap.melt$count<-melt(overlap$countTable,id=1)$value
# overlap.melt.summary2<-overlap.melt[!overlap.melt$Var2=="0",]
# 
# for(i in 28:30) {
# overlap<-GOseq.overlapTable(test[,"ssom3.4.unit.classif"],test[,i],genes=test$AGI)
# overlap.melt<-melt(overlap$pTable,id=1) #
# overlap.melt$count<-melt(overlap$countTable,id=1)$value
# overlap.melt<-overlap.melt[!overlap.melt$Var2=="0",]
# overlap.melt.summary2<-rbind(overlap.melt.summary2,overlap.melt)
# }
# overlap.melt.summary2[,"value"]<--log10(overlap.melt.summary2[,"value"])
# overlap.melt.summary2[overlap.melt.summary2$value>10,"value"]<-10
# overlap.melt.summary2[overlap.melt.summary2$count==0,"count"]<-""
# # Add (111916)
# overlap.melt.summary<-rbind(overlap.melt.summary,overlap.melt.summary2)
...
save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.111916.Rdata") # whitney. 
...
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.111616.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.111616.png",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") # testing this with Whitney (112816)
...
library(WGCNA);library(ggplot2);library(reshape2);library(scales)
# custom categories
load("../../Nozue2016_SAStranscriptome_data/input/hormone.responsiveness5.Rdata")
########################
## change list into data.frame
hormone.responsiveness5.DF<-data.frame(t(sapply(hormone.responsiveness5,c))) # modified for ver5
names(hormone.responsiveness5.DF)<-attributes(hormone.responsiveness5)$category
...
# merge with WGCNA modules
test<-merge(hormone.responsiveness5.DF,SAS.expression.vst.s.kazu.largeCV.modules[,c("AGI","1")],by="AGI",all=TRUE)
...
load("../../Nozue2016_SAStranscriptome_output/output/custom.category.v4.overlap.overlap.melt.summary.sGOseq.Rdata")
...
#ggsave(q4,file="Col.WGCNA.modules.vs.GenotypeSpecific.genes.table.pdf",width=11,height=8)
ggsave(q4,file="Fig6_Col.WGCNA.modules.vs.my.category.table.s.111516.pdf",width=11,height=8,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
ggsave(q4,file="Fig6_Col.WGCNA.modules.vs.my.category.table.s.111516.png",width=11,height=8,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
...
categories$my.category<-factor(categories$my.category,levels=rev(c("IAAup","BLup","BLdown","PIFtarget","ACCup","ABAup","ABAdown","MJup","MJdown","MYC234up","SARplus","NPR1down","WRKY33up"))) # 111816
