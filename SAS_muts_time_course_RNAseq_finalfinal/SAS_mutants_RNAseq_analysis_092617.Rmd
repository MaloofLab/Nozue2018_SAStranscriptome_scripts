...
#source(file.path(homedir,"SAS_muts_time_course_RNAseq_finalfinal","function_RNAseq_time_course.R"))
#source("SAS_muts_time_course_RNAseq_finalfinal/function_RNAseq_time_course.R")
```
...
# genotype version
PLoSGenetics2015mutants<-samples.nolow5[samples.nolow5$genotype %in% c("1","17","19","11","4","5","3","14","10","12","6","13","7","8","16","9"),]
...
PLoSGenetics2015mutants.errrDE<-PLoSGenetics2015mutants[!PLoSGenetics2015mutants$genotype %in% errorDE,]

save(PLoSGenetics2015mutants.errrDE,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","PLoSGenetics2015mutants.errrDE.Rdata"))
#setwd("../../Nozue2016_SAStranscriptome_data/input/")
# data <- read.delim("SAS_counts_merged_updated_final.bam.tsv",sep="\t",row.names=2)
#counts<-read.table(file.path(homedir,"..","..","Nozue2016_SAStranscriptome_data/input/SAS_counts_merged_updated_final.bam.tsv"),header=TRUE) # use SAS_counts_merged_updated.bam.tsv (081514)

#data<-data[,colnames(data) %in% PLoSGenetics2015mutants.errrDE$file] # data?
counts<-counts[,colnames(counts) %in% PLoSGenetics2015mutants.errrDE$file]
...
vstMat<-read.csv("kazu.SAS.expression.VST.092517.csv")
...
SAS.expression.vst<-read.csv(file.path("..","..","Nozue2016_SAStranscriptome_data","input","kazu.SAS.expression.VST.092517.csv"),row.names=1) # new transformed data by DEseq2 (082016), (092617)

#rownames(SAS.expression.vst)<-SAS.expression.vst$X
Genotype<-levels(as.factor(sub("(C|D|E)([[:digit:]]+)(L|H)(1|4|25|49)(A.merged.bam)","\\2",names(SAS.expression.vst)))) # 13 genotypes inclucing Col
#
for(i in Genotype) { # why I eliminate genotype 1 and 10???
  ...
  pdf(file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h",paste("BCV_MDS_with_genotype_wo_outlier_wo16h",i,".pdf",sep="")),width=11,height=8) # fine name fixed (122214)
  par(mfrow=c(1,2))
  plotBCV(dge.nolow.s.batch.glm,main=paste("genotype",i))
  plotMDS(dge.nolow.s.batch.glm,main=paste("genotype",i))
  dev.off() 
  ...
  #save(DE.batch.anytime,file=paste("../../Nozue2016_SAStranscriptome_output/output/DE.batch.anytime_genotype_wo_outlier_wo16h",i,".Rdata",sep="")) # file name fixed (122114)   
    save(DE.batch.anytime,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output",paste("DE.batch.anytime_genotype_wo_outlier_wo16h",i,".Rdata",sep=""))) # file name fixed (122114) , (092617)   
}
```

# summary of DE genes 
* (053014 - 053114): revised version (081514 - );more revision (060415 - ); more (092617)
```{r eval=FALSE}
getwd()
...
for(i in DEfiles[-1]) {
  ...
  if(dim(DE.batch.anytime)[1]>1) {
    rownames(DE.batch.anytime)<-paste(rownames(DE.batch.anytime),gsub("(DE.batch.anytime_genotype_wo_outlier_wo16h)([[:print:]]+)(.Rdata)","\\2",i),sep=".") # fixed, 112616
    temp<-rbind(temp,DE.batch.anytime)  }
  else next
}
DGEsummary.edgeR.response<-temp
tail(DGEsummary.edgeR.response)
dim(DGEsummary.edgeR.response) #755   8 for final sequence data with normalization (081514)
save(DGEsummary.edgeR.response,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","DGEsummary.edgeR.response.Rdata"))
# # genotype conversion 
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","samples.nolow5.Rdata"))
samples.nolow5
samples.nolow5$genotype2<-""
...
#setwd(homedir)
```
## select genes with largeCV and calculate responsiveness for SOM analysis, updated 082016, 092617
```{r eval=FALSE}
#setwd(homedir)
...
sum(as.integer(SAS.expression.vst.s.kazu$cv>4))/dim(SAS.expression.vst.s.kazu)[1] #[1] 0.830806
sum(as.integer(SAS.expression.vst.s.kazu$cv>4.5))/dim(SAS.expression.vst.s.kazu)[1] #[1] 0.3621328
sum(as.integer(SAS.expression.vst.s.kazu$cv>5))/dim(SAS.expression.vst.s.kazu)[1] #[1] 0.1642813
...
dim(SAS.expression.vst.s.kazu.largeCV) #[1] 7438  403 > 18379   264 > 11057   288
...
SAS.expression.vst.s.kazu.largeCV # this is not response
#
...
#load("../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/DGEsummary.edgeR.response.Rdata") #"AT1G33925load("../../Nozue2016_SAStranscriptome_output/output/DGEsummary.edgeR.response.Rdata") #"AT1G33925.1" (112616 fixed)
...
length(DGEgenelist.edgeRglm) # 418 > 363
# 
...
save(DGE.summary.vst.response.kazu.largeCV,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h","DGE.summary.vst.response.kazu.largeCV.Rdata")) # updated 052916, 092517
...
write.csv(SOMclusters.description,"../../Nozue2016_SAStranscriptome_output/figs_tables/TableS3_SOMclusters.description092617.csv") #090116
...
length(unique(newdata.vst$AGI)) # 333 genes (111816)
# boxplot
...
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","newdata.vst.Rdata")) # 122214, 092617
...
#newdata.vst.Col.melt$ssom3.4.unit.classif<-factor(newdata.vst.Col.melt$ssom3.4.unit.classif,levels=c(10, 7,9,2,12,8,4,1,5,3,11,6)) # use this factor for all heatmap. start from early peak or troph to late peak or troph as manuscript described (112816)
newdata.vst.Col.melt$ssom3.4.unit.classif<-factor(newdata.vst.Col.melt$ssom3.4.unit.classif,levels=c(3,2,12,6,4,9,7,1,5,10,8,11)) # use this factor for all heatmap. start from early peak or troph to late peak or troph as manuscript described (092617)
...
#ggsave("Fig2_Coexpression_clusters_of_shade-responsive_genes_112816.pdf",width=11,height=5,unit="in",path="../../Nozue2016_SAStranscriptome_output/figs_tables") # needs to change order of cluster (112816). File name changed (091016; 110716)
ggsave("Fig2_Coexpression_clusters_of_shade-responsive_genes_092617.pdf",width=11,height=5,unit="in",path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables")) # needs to change order of cluster (112816). File name changed (091016; 110716)

# To which clusters in mutants do genes in Col cluster 3 belong?
newdata.vst$gt2<-vlookup(newdata.vst$gt,conversion.table,2)
...
# GOseq anaysis of SOM clusters (022215; 092617)
```{r eval=FALSE}
load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","newdata.vst.Rdata")) #
newdata.vst.Col<-newdata.vst[newdata.vst$gt==1,]
...
```
# write summary of GOseq analsyis of SOM clusters (022215; 092617)
```{r eval=FALSE}
#rm(list=ls())
...
write.csv(SOM.Col.cluster.GOseq.summary,file=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","TableS2_GO_enrichment_analysis_of_co-expressed_SOM_clusters_of_shade-responsive_genes092617.csv")) # 091016,092617
```
...
for(i in Genotype[-1]) {# Genotype 17, 3,4,5,6,7,9 exceed 2000 genes. List up 5000 genes
  ...
  dim(DE.batch3.gt) #
  save(DE.batch3.gt,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h","genotype_specific5",paste("DE.genotype.batch.sun.gt.wo16h",i,".Rdata",sep=""))) 
  # save  dge.nolow.s.batch3.glm.lrt
    save(dge.nolow.s.batch3.glm.lrt,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h","genotype_specific5",paste("dge.nolow.s.batch3.glm.lrt.sun",i,".Rdata",sep=""))) # added this line (try to use this object for coef, 061715)
}
getwd()
```
...
for(i in Genotype[-1]) {# Genotype 17, 3,4,5,6,7,9 exceed 2000 genes. List up 5000 genes
  ...
  DE.batch3.gt<-topTags(dge.nolow.s.batch3.glm.lrt,n=10000)$table[topTags(dge.nolow.s.batch3.glm.lrt,n=10000)$table$FDR<0.001,] # 0.05 for genotype_specific2 which gave me 5000 genes in hy5 (genotype3)
  dim(DE.batch3.gt) #
  save(DE.batch3.gt,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h","genotype_specific5",paste("DE.genotype.batch.shade.gt.wo16h",i,".Rdata",sep="")))  
  save(dge.nolow.s.batch3.glm.lrt,file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","edgeR","outlier_wo16h","genotype_specific5",paste("dge.nolow.s.batch3.glm.lrt.shade",i,".Rdata",sep=""))) # added this line (try to use this object for coef, 061715)
}
...
# sun
#setwd(homedir)
DE.genotype.batch.sun.files.kazu<-list.files(pattern="DE.genotype.batch.sun",path="../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/genotype_specific5/") # needs to select "sun" object (060615)
...
# shade
DE.genotype.batch.shade.files.kazu<-list.files(pattern="DE.genotype.batch.shade",path="../../Nozue2016_SAStranscriptome_output/output/edgeR/outlier_wo16h/genotype_specific5/")
## load the first file
...
# 091016, 092617
DE.genotype.sun<-gsub("(DE.genotype.batch.sun.gt.wo16h)([[:digit:]]+)(.Rdata)","\\2",DE.genotype.batch.sun.files.kazu)
DE.genotype.shade<-gsub("(DE.genotype.batch.shade.gt.wo16h)([[:digit:]]+)(.Rdata)","\\2",DE.genotype.batch.shade.files.kazu)
...
DE.genotype.batch.genes.unique.DF
...
# adding gene description
#TAIR10_gene_descriptions<-read.csv("../../Nozue2016_SAStranscriptome_data/input/TAIR10_functional_descriptions.csv")  # move this part in function
...
write.csv(DE.genotype.batch.genes.SOMclusters.description,"../../Nozue2016_SAStranscriptome_output/figs_tables/Dataset3_Complete_list_of_shade-responsive_genes_and_SOM_gene_clusters_and_misexpressed_genes092617.csv")
```
# SOM vs mutant affected genes overlapTable
```{r}
...
save(overlap.GOseq.melt.summary, file=file.path("..","..","Nozue2016_SAStranscriptome_output","output","overlap.GOseq.melt.summary.SOM.DEgenes.092617.Rdata") )

load(file.path("..","..","Nozue2016_SAStranscriptome_output","output","overlap.GOseq.melt.summary.SOM.DEgenes.110716.Rdata"))
# # graph
...
overlap.GOseq.melt.summary.ss$Var1<-factor(overlap.GOseq.melt.summary.ss$Var1,levels=c(10, 7,9,2,12,8,4,1,5,3,11,6)) # 112816
...
ggsave(q4b,file="Fig4_Shade_avoidance_mutants_misexpress_specific_shade-response_SOM_cluster_111616.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") # running in whitney (111616)
...
#load("../../Nozue2016_SAStranscriptome_data/input/hormone.responsiveness5.Rdata") #new version. 
load("../../Nozue2016_SAStranscriptome_data/input/hormone.responsiveness6.Rdata") #corrected version. (113016)
...
write.csv(hormone.responsiveness.SOMclusters.description,file="../../Nozue2016_SAStranscriptome_output/figs_tables//Dataset2_Custom_enrichment_analysis_of_co-expressed_SOM_clusters_of_shade-responsive_genes.csv") #needs to revise (112816)

# in whitney (111016) for ORA
...
# overlap table 
i<-2 
overlap<-GOseq.overlapTable(test[,"ssom3.4.unit.classif"],test[,i],genes=test$AGI)
...
save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.113016.Rdata") # whitney. 

setwd(homedir)
#load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.110816.Rdata")
# fix reverse of WRKY33up and WRKY33down 
# overlap.melt.summary$Var2<-as.character(overlap.melt.summary$Var2)
# overlap.melt.summary$Var2<-gsub("WRKY33up","WRKY33down_true",overlap.melt.summary$Var2)
# overlap.melt.summary$Var2<-gsub("WRKY33down$","WRKY33up_true",overlap.melt.summary$Var2)
# overlap.melt.summary$Var2<-gsub("_true","",overlap.melt.summary$Var2)
# save(overlap.melt.summary,file="../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.113016.Rdata") # whitney. 
...
overlap.melt.summary.s$Var1<-factor(overlap.melt.summary.s$Var1,levels=c(10, 7,9,2,12,8,4,1,5,3,11,6)) # 112816
...
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.113016.pdf",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables")
ggsave(q4b,file="Fig3_Col.SOM.clusters.vs.my.category.table.s.2.GOseq.113016.png",width=13,height=6,path="../../Nozue2016_SAStranscriptome_output/figs_tables") # 
...
setwd("../../Nozue2016_SAStranscriptome_output/output/")
moduleColors.files<-list.files(pattern=".largeCV.merge.net32.RData") # for  merged modules by mergeCloseModules function above for Col (110816); newer (110916)
### use merged modules by mergeCloseModules function above for Col (22 modules; 110716)
load("all1.largeCV.merge.net32.RData")
load("../../Nozue2016_SAStranscriptome_output/output/all1.largeCV.merge.net32.RData") # does not work
...
## check module character by GO ORA
...
save(SAS.expression.vst.s.kazu.largeCV.modules,file="../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules121316.Rdata") #121316
...
```{r,eval=FALSE}
setwd(homedir)
load("../../Nozue2016_SAStranscriptome_outpu/output/DE.genotype.batch.genes.unique.DF.Rdata")
#setwd(homedir)
# load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules.Rdata")
#load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules110916.Rdata")
load("../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules121316.Rdata")
...
table(DE.genotype.batch.genes.WGCNAmodules$WGCNAmodules) # "salmon" should be 182, "orange4" should be 50
...
write.csv(DE.genotype.batch.genes.WGCNAmodules.description,file="../Nozue2016_SAStranscriptome_output/figs_tables/Dataset7_Complete_gene_list_of_genes_misexpressed_in_mutant_and_found_in_WGCNA_modules_visualized_in_Fig7.csv") #091616. Column names were screwed up (111616). Orange > "salmon", "salmon" > "orange4" (121316)

# in whitney
#test<-read.csv("../Nozue2016_SAStranscriptome_output/figs_tables/Dataset7_Complete_gene_list_of_genes_misexpressed_in_mutant_and_found_in_WGCNA_modules_visualized_in_Fig7.csv") # does not work (091817)
test<-read.csv("../Nozue2016_SAStranscriptome_output/figs_tables/Dataset7.csv") # does  work (091817)
...
overlap.melt.summary$Var2<-droplevels(overlap.melt.summary$Var2)
#table(overlap.melt.summary[,"value"])
# graph
#overlap.melt.summary.s<-overlap.melt.summary[!overlap.melt.summary$Var2 %in% c("bsk5_1.sun","bsk5_1.shade","AT5G02540_1.sun","AT5G02540_1.shade"),]
...
# load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.WGCNA.mut.GOseq.Rdata") 
load("../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.WGCNA.mut.GOseq.Rdata") # 062717
load("overlap.melt.summary.WGCNA.mut.GOseq.Rdata") # 062717, 121616

overlap.melt.summary
overlap.melt.summary.ss<-overlap.melt.summary[!overlap.melt.summary$Var1=="0",]
...
ggsave(q4,file="Fig7_Col.WGCNA.modules.vs.GenotypeSpecific.genes.table.s.GOseq121316.pdf",width=11,height=8,path="../Nozue2016_SAStranscriptome_output/figs_tables")
ggsave(q4,file="Fig7_Col.WGCNA.modules.vs.GenotypeSpecific.genes.table.s.GOseq121316.png",width=11,height=8,path="../Nozue2016_SAStranscriptome_output/figs_tables")
...
#setwd(homedir)
# load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules.Rdata")
#load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules110816.Rdata") # tan is SA and JA an everything. too big. Also overlapped with all mutants (Fig6)
# lower threshold in meregeCloseModules.
# load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules110916.Rdata") # 
#load("../../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules121316.Rdata") # 
load("../Nozue2016_SAStranscriptome_output/output/SAS.expression.vst.s.kazu.largeCV.modules121316.Rdata") # 091817

# setwd("../../Nozue2016_SAStranscriptome_output/output/")
#setwd("Nozue2016_SAStranscriptome_output/output/") # 091817
...
for(x in modules) {
  ...
  save(temp.GOseq,temp.genes,file=paste("../Nozue2016_SAStranscriptome_output/output/WGCNA.Col.module.",x,".GOseq.Rdata",sep=""))
}
  } # 110916, 121316, 091817
```
## write summary of WGCNA.Col.module.GOseq 
```{r eval=FALSE}
#rm(list=ls())
#setwd("../../Nozue2016_SAStranscriptome_output/output/")
WGCNA.Col.module.GOseq.files<-list.files(path="../Nozue2016_SAStranscriptome_output/output/",pattern="^WGCNA.Col.module.")
WGCNA.Col.module.GOseq.files<-grep(".GOseq.Rdata",WGCNA.Col.module.GOseq.files,value=TRUE)

load(file.path("..","Nozue2016_SAStranscriptome_output","output",WGCNA.Col.module.GOseq.files[1])) # using file.path 091817
...
write.csv(WGCNA.Col.module.GOseq.summary,file="../Nozue2016_SAStranscriptome_output/figs_tables/Dataset5_GO_enrichment_analysis_of_WGCNA_modules091817.csv") # Dataset 5. GO enrichment analysis of WGCNA gene co-expression modules (091716, salmon4<>orange 121316, rerun 091817)
```
...
} # 121316 with salmon4<>orange fixed module names
#setwd(homedir)
```
...
```
# overlap analysis with custome categories (WGCNA modules): Fig6
```{r eval=FALSE}
library(WGCNA);library(ggplot2);library(reshape2);library(scales)
# custom categories
#load("../../Nozue2016_SAStranscriptome_data/input/hormone.responsiveness6.Rdata")
load("../Nozue2016_SAStranscriptome_data/input/hormone.responsiveness6.Rdata")

########################
## change list into data.frame
hormone.responsiveness6.DF<-data.frame(t(sapply(hormone.responsiveness6,c))) # modified for ver6
names(hormone.responsiveness6.DF)<-attributes(hormone.responsiveness6)$category
...
# merge with WGCNA modules
test<-merge(hormone.responsiveness6.DF,SAS.expression.vst.s.kazu.largeCV.modules[,c("AGI","1")],by="AGI",all=TRUE)
...
# salmon genes and SA, npr1 responsiveness? (PCC1, XBAT34, AIG1, ACD6)
hormone.responsiveness.WGCNAmodules[hormone.responsiveness.WGCNAmodules$WGCNAmodules %in% "salmon" & hormone.responsiveness.WGCNAmodules$AGI %in% c("AT3G22231","AT4G14365","AT1G33960","AT4G14400"), c(1,18:19,24:34)]

#setwd("../../Nozue2016_SAStranscriptome_output/output/")
hormone.responsiveness.WGCNAmodules<-read.csv("../Nozue2016_SAStranscriptome_output/figs_tables/Supplemental_Dataset_6_Custom_category_enrichment_analysis_of_WGCNA_gene_co-expression_modules.csv")
...
i<-2
overlap<-GOseq.overlapTable(test[,"WGCNAmodules"],test[,i],genes=test$AGI) # default permutaiton is 2000 in goseq(method="Sampling")
overlap.melt<-melt(overlap$pTable,id=1) #
overlap.melt$count<-melt(overlap$countTable,id=1)$value
...
load("Nozue2016_SAStranscriptome_output/output/custom.category.v4.overlap.overlap.melt.summary.s.GOseq.Rdata") # 062717

# fix reverse of WRKY33up and WRKY33down 
# custom.category.overlap.melt.summary.s$Var2<-as.character(custom.category.overlap.melt.summary.s$Var2)
# custom.category.overlap.melt.summary.s$Var2<-gsub("WRKY33up","WRKY33down_true",custom.category.overlap.melt.summary.s$Var2)
# custom.category.overlap.melt.summary.s$Var2<-gsub("WRKY33down$","WRKY33up_true",custom.category.overlap.melt.summary.s$Var2)
# custom.category.overlap.melt.summary.s$Var2<-gsub("_true","",custom.category.overlap.melt.summary.s$Var2)
# save(custom.category.overlap.melt.summary.s,file="../../Nozue2016_SAStranscriptome_output/output/custom.category.v4.overlap.overlap.melt.summary.s.GOseq.Rdata") 
...
dim(DE.genotype.batch.genes.WGCNAmodules.description) # 17985   28
...
categories <- read.csv("../../Nozue2016_SAStranscriptome_output/figs_tables/TableS7_test.wozero.melt.hormone.significant.sss.113016.csv",row.names=1,as.is=TRUE) # does not work.
summary(categories)
head(categories)
...
my.category.heatmap.graph[[i]]<-my.category.heatmap5(summary3.response=summary.vst.response.kazu,gt.num=i,gt2=conversion.table[conversion.table$num==i,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.v11.png",w=5,h=5*4/3, path="../../Nozue2016_SAStranscriptome_output/figs_tables/",save.plot=F,legend.fill=T) # error
}
...
my.category.diff.data[[i]]<-my.category.diff(summary3.response=summary.vst.response.kazu,gt.num=i,gt2=conversion.table[conversion.table$num==i,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.v12.png",w=5,h=5*4/3, path="../../Nozue2016_SAStranscriptome_output/figs_tables/",save.plot=F,legend.fill=T) # make specialized for difference heatmap function
}
...
my.category.diff.heatmap.graph[[i]]<-my.category.diff.heatmap1(summary3.response=summary.vst.response.kazu,gt.num=i,gt2=conversion.table[conversion.table$num==i,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.v8.png",w=5,h=5*4/3, path="../../Nozue2016_SAStranscriptome_output/figs_tables/",save.plot=F,legend.fill=T,max.min.value=max.min) # make specialized for difference heatmap function
}
# significance
# write how to calculate "Fig5B_response_p_values.csv"

# read p value
response.p.values<-read.csv("../../Nozue2016_SAStranscriptome_output/figs_tables/Fig5B_response_p_values.v2.csv") # from modified Julin's script (Kazu_test_v2.R)
response.p.values.melt<-melt(response.p.values[,-1],id="gt") # this is wrong?

for(i in c(17,19,3,4,5,14,10,6,13,7,8,9)) {
significant.position<-data.frame(category=c("IAAup","BLup","BLdown","PIFtarget","ACCup","ABAup","ABAdown","MJup","MJdown","SAup_24h"),x=0,y=1:10,symbol.gt="",symbol.gt.time="")
...
my.category.heatmap.all <- plot_grid(my.category.heatmap.graph[[1]],ggplot()+geom_blank(),ggplot()+geom_blank(),my.category.diff.heatmap.graph[[17]],my.category.diff.heatmap.graph[[19]],my.category.diff.heatmap.graph[[3]],my.category.diff.heatmap.graph[[4]],my.category.diff.heatmap.graph[[5]],my.category.diff.heatmap.graph[[14]],my.category.diff.heatmap.graph[[10]],my.category.diff.heatmap.graph[[6]],my.category.diff.heatmap.graph[[13]],my.category.diff.heatmap.graph[[7]],my.category.diff.heatmap.graph[[8]],my.category.diff.heatmap.graph[[9]],
                      ...
                      label_size = 50)
setwd("../../Nozue2016_SAStranscriptome_output/figs_tables/")
save_plot("Fig5_all.my.category.diff.expression.pattern.v6.120116.FDR0.01.png", my.category.heatmap.all,
...
#
setwd("../../Nozue2016_SAStranscriptome_output/figs_tables/")
save_plot("Fig_S2_all.my.category.expression.pattern.v11.120116.png", my.category.heatmap.all,
          ncol = 3, # we're saving a grid plot of 2 columns
          nrow = 5, # and 2 rows
          base_aspect_ratio = 2.5,
          base_height=6,
...
# fold chagne (shade responsiveness)
load("../../Nozue2016_SAStranscriptome_output/output/summary.vst.response.Rdata") 
# summary.vst.response.kazu<-summary.vst.response[,gsub("([[:digit:]]+)(_)(1|4|16|25|49)(hrA)","\\1",colnames(summary.vst.response)) %in% 1:19]
...
# newone
# load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.110816.Rdata") # why this is not all categories?
#load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.111916.Rdata") 
load("../../Nozue2016_SAStranscriptome_output/output/overlap.melt.summary.SOM.custom.GOseq.113016.Rdata") #WRKY33 up and down fixed 
...
overlap.melt.summary.s.significant<-overlap.melt.summary.s[overlap.melt.summary.s$value> -log10(0.05),] # pvalue<0.05
overlap.melt.summary.s.significant$category.cluster<-with(overlap.melt.summary.s.significant,paste(Var2,Var1,sep="_"))
...
test.wozero.melt.hormone.significant<-test.wozero.melt[test.wozero.melt$category.cluster %in% overlap.melt.summary.s.significant$category.cluster,]
...
#test.wozero.melt.hormone.significant.s$my.category<-factor(test.wozero.melt.hormone.significant.s$my.category,levels=rev(c("IAAup","BLup","BLdown","PIFtarget","ACCup","ABAup","ABAdown","MJup","MJdown","MYC234up","SARplus","NPR1down","WRKY33up"))) # remove GAdown (052616)
#write.csv(test.wozero.melt.hormone.significant.s,file="../../Nozue2016_SAStranscriptome_output/output/test.wozero.melt.hormone.significant.s.csv") # needs to revise
...
# write.csv(test.wozero.melt.hormone.significant.s,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS7_test.wozero.melt.hormone.significant.s.113016.csv") # needs to revise?
write.csv(test.wozero.melt.hormone.significant.ss,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS7_test.wozero.melt.hormone.significant.ss.113016.csv") # 
...
setwd("../../Nozue2016_SAStranscriptome_output/output/")
#write.csv(test.wozero.melt.hormone.significant.s.summary,file="test.wozero.melt.hormone.significant.s.summary.csv")
setwd(homedir)
...
## 
reshape(test.wozero.melt.hormone.significant.ss,idvar="AGI",timevar="my.category",direction="wide")
test.wozero.melt.hormone.significant.ss.summary<-table(test.wozero.melt.hormone.significant.s)[,-1]
setwd("../../Nozue2016_SAStranscriptome_output/output/")
#write.csv(test.wozero.melt.hormone.significant.ss.summary,file="test.wozero.melt.hormone.significant.ss.summary.csv")
setwd(homedir)
# genes with no category
test.wozero.melt.hormone.significant.ss.summary.zero<-rownames(test.wozero.melt.hormone.significant.ss.summary[rowSums(test.wozero.melt.hormone.significant.ss.summary)<1,]) 
# 
test.wozero.melt.hormone.significant.sss<-test.wozero.melt.hormone.significant.ss[!test.wozero.melt.hormone.significant.ss$AGI %in% test.wozero.melt.hormone.significant.ss.summary.zero,]
write.csv(test.wozero.melt.hormone.significant.sss,file="../../Nozue2016_SAStranscriptome_output/figs_tables/TableS7_test.wozero.melt.hormone.significant.sss.113016.csv") # 

```
# appendix 2  "Should I filter probesets or genes for WGCNA analysis?
