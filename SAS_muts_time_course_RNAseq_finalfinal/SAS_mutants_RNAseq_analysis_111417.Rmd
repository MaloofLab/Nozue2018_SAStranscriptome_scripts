...
my.category.heatmap.all <- plot_grid(my.category.heatmap.graph[[1]]+theme(legend.position="none"), 
                      ...
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=5,
                      label_size = 50) # no "A" and "B" because they overlap with graphs. Label them later in Illustlator (020818)
save_plot(file.path("..","..","Nozue2016_SAStranscriptome_output","output","tsne_dbscan_overlapTable_heatmap",paste("Supplemental_Dataset7_all.my.category.expression.pattern.v16.log2.signif.v3.",title,".png",sep="")), my.category.heatmap.all,
...
# eps3.25_MinPts14.set45 (020818)
system(paste("cp ",file.path("..","..","Nozue2016_SAStranscriptome_output","output","tsne_dbscan_overlapTable_heatmap","Supplemental_Dataset7_all.my.category.expression.pattern.v16.log2.signif.v3.eps3.25_MinPts14.set45.png")," ", file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Fig2_all.my.category.expression.pattern.v16.log2.signif.v3.eps3.25_MinPts14.set45.png"),sep=""))
```
# archivre Supplemental_Dataset7 files 
```{r}
# zip files (not used because they were too big (~1G)
# system(paste("zip ",file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables","Supplemental_Dataset7.zip")," ", file.path("..","..","Nozue2016_SAStranscriptome_output","output","tsne_dbscan_overlapTable_heatmap","Supplemental_Dataset4_all.my.category.expression.pattern.v16.log2.signif.v3.*"),sep=""))
```
...
pdf.files<-list.files(path="converted.pdfs")
png.names<-paste(gsub("([[:print:]]+)(.pdf)","\\1",pdf.files),".png",sep="")
jpg.names<-paste(gsub("([[:print:]]+)(.pdf)","\\1",pdf.files),".jpg",sep="")

system(paste("sips -s format png ",file.path(".","converted.pdfs",pdf.files)," --out ", file.path(".","converted.pdfs.pngs",png.names),sep=""))
# only one file
system(paste("sips -s format png ",file.path(".","converted.pdfs",pdf.files[1])," --out ", file.path(".","converted.pdfs.pngs",png.names[1]),sep=""))
# resize
system(paste("sips -z 600 800 -s format png ",file.path(".","converted.pdfs",pdf.files[2])," --out ", file.path(".","converted.pdfs.pngs",png.names[2]),sep=""))
# all
for(i in 1:length(pdf.files)) {
system(paste("sips -z 600 800 -s format png ",file.path(".","converted.pdfs",pdf.files[i])," --out ", file.path(".","converted.pdfs.pngs",png.names[i]),sep=""))
}
# all jpg (not super reduced size)
for(i in 1:length(pdf.files)) {
system(paste("sips -z 600 800 -s format jpeg ",file.path(".","converted.pdfs",pdf.files[i])," --out ", file.path(".","converted.pdfs.jpgs",jpg.names[i]),sep=""))
}
# convert resultant png into pdf
for(i in 1:length(pdf.files)) {
system(paste("sips -s format pdf ",file.path(".","converted.pdfs.pngs",png.names[i])," --out ", file.path(".","converted.pdfs.pngs.pdfs",pdf.files[i]),sep=""))
}
# convert resultant jpg into pdf (not much reduced size)
for(i in 1:length(pdf.files)) {
system(paste("sips -s format pdf ",file.path(".","converted.pdfs.jpgs",jpg.names[i])," --out ", file.path(".","converted.pdfs.jpgs.pdfs",pdf.files[i]),sep=""))
}
...
# plot
q4<-ggplot(overlap.melt.summary.ss,aes(y=factor(overlap.melt.summary.ss$Var1,levels=rev(c("phyABDE_up","phyABDE_down","Sessa.1h.up","Sessa.1h.down"))),x=factor(Var2))) # sort row
q4 <- q4 + geom_tile(color="black",aes(fill=value))+scale_fill_gradient2(low="white", mid="white",high="magenta",midpoint=2) # Is this better? (061716)
#q4 <- q4 +theme(legend.position="none") # this removed all legend
q4 <- q4 + theme(axis.text.x=element_text(angle=90,hjust=1),
                 axis.ticks = element_line(size=0,color="white"),
...
table(overlap.melt.summary.ss$Var2)
# plot
q4<-ggplot(overlap.melt.summary.ss,aes(y=factor(Var1),x=factor(Var2))) # sort row
q4 <- q4 + geom_tile(color="black",aes(fill=value))+scale_fill_gradient2(low="white", mid="white",high="magenta",midpoint=2) # Is this better? (061716)
#q4 <- q4 +theme(legend.position="none") # this removed all legend
q4 <- q4 + theme(axis.text.x=element_text(angle=90,hjust=1),
                 axis.ticks = element_line(size=0,color="white"),
...
q4 <- q4 + labs(x="custom category",y="",title="",fill="-log10\n pvalue")
#q4 <- q4 + theme(legend.title=element_blank())
ggsave(q4,file="Supplemental_FigX.phyB.adult.my.category.040618.pdf",width=11,height=4,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"))
ggsave(q4,file="Supplemental_FigX.phyB.adult.my.category.040618.png",width=11,height=4,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"))
...
table(overlap.melt.summary.ss$Var1)
# plot
SupF.S1B<-ggplot(overlap.melt.summary.ss,aes(y=Var2,x=factor(Var1))) 
SupF.S1B <- SupF.S1B + geom_tile(color="black",aes(fill=value))+scale_fill_gradient2(low="white", mid="white",high="magenta",midpoint=2) 
SupF.S1B <- SupF.S1B + theme(axis.text.x=element_text(angle=90,hjust=1),
                 axis.ticks = element_line(size=0,color="white"),
                 axis.line=element_line(size=1)) # adjust axis line width
SupF.S1B <- SupF.S1B + geom_text(aes(label=count),size=4)
SupF.S1B <- SupF.S1B + labs(x="WGCNA category",y="",title="",fill="-log10\n pvalue")
SupF.S1B
## 
# ggsave(SupF.S1B,file="Supplemental_Fig5.phyABDE.Sessa1h.vs.WGCNA.040618.pdf",width=11,height=4,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"))
# ggsave(SupF.S1B,file="Supplemental_Fig5.phyABDE.Sessa1h.vs.WGCNA.040618.png",width=11,height=4,path=file.path("..","..","Nozue2016_SAStranscriptome_output","figs_tables"))
```
# Making a composit fig 
```{r}
library(cowplot)
