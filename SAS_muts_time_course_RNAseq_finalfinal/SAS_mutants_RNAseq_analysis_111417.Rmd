...
## preparation
```{r}
# change working directory (see below)
...
my.category.heatmap.graph[[1]]<-my.category.heatmap6(summary3.response=summary.vst.response.kazu,gt.num=1,gt2=conversion.table[conversion.table$num==1,2],newdata.Col.wGO.selected=categories,plotname="my.category.expression.pattern.102617v1.png",w=5,h=5*4/3, path=file.path("..","..","Nozue2016_SAStranscriptome_output","output","tsne_dbscan_overlapTable_heatmap"),save.plot=F,legend.fill=T,file.name=paste("v3",title,sep=""),category.level=levels(categories$my.category)) + theme(legend.direction = "horizontal",legend.text=element_text(angle=90),legend.key.width=unit(2,"inch"), legend.text.align=1,legend.justification=c(0,0)) # +labs(fill="Log2 fold change\n (shade/sun)") # +guides(fill=guide_legend(title.position = "top")) ,legend.title.align=1,legend.justification=c(0,0) did not work. Below adding legend title by using draw_text(). (092418)
...
my.category.heatmap.Fig2A <- plot_grid(my.category.heatmap.graph[[1]],ggplot()+geom_blank(),  ggplot()+geom_blank(),                            
                      vjust = 1.1, hjust=-0.5,
                      ncol=3,nrow=1,
                      # rel_widths=c(1.275,0.725,1),
                      rel_widths=c(1.64,0.36,1),
...
my.category.heatmap.Fig2B<-plot_grid(my.category.heatmap.graph[[17]],my.category.heatmap.graph[[19]],my.category.heatmap.graph[[3]],my.category.heatmap.graph[[4]],my.category.heatmap.graph[[5]],my.category.heatmap.graph[[14]],my.category.heatmap.graph[[10]],my.category.heatmap.graph[[6]],my.category.heatmap.graph[[13]],my.category.heatmap.graph[[7]],my.category.heatmap.graph[[8]],my.category.heatmap.graph[[9]],
                      ...
                      ncol=3,nrow=4,
                      label_size = 10) 
my.category.heatmap.all2<-plot_grid(my.category.heatmap.Fig2A,my.category.heatmap.Fig2B,rel_heights=c(0.92,4),ncol=1,nrow=2)+ draw_text(title,x=0.75,y=0.95,size=50) + draw_text("Log2 fold change\n (shade/sun)",x=0.45,y=0.955,size=50) # adding legend title by using draw_text()
...
# eps3.25_MinPts14.set45 (020818; Plant Physiol version 091118; horizontal legend version 092418)
...
system(paste("mkdir ",file.path("..","..","Nozue2016_SAStranscriptome_output","output","reduced.pngs"),sep=""))
system(paste("mkdir ",file.path("..","..","Nozue2016_SAStranscriptome_output","output","reduced.pdfs"),sep=""))
system(paste("sips -z 600 800 -s format png ",file.path("..","..","Nozue2016_SAStranscriptome_output","output","pngs",png.files[2])," --out ", file.path("..","..","Nozue2016_SAStranscriptome_output","output","reduced.pngs",png.files[2]),sep="")) # works
...
# combine those pdf into one pdf using Autometor (22.4 M with -z 600 800 for png). Select files and find "combinePDFs under Action button"
# add legend for manuscript submission to Plant Physiol by using Illustrator.
```
